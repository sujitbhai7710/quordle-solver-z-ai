// compare-30.mjs - 30-game fair comparison (both with recursive lookahead)
...
// ============================================================
// WORDBOT-STYLE WITH RECURSIVE LOOKAHEAD (pure JS)
// ============================================================
function cp(g, a) {
...
// Recursive lookahead (same as our Rust solver)
    ...
    for (let i = 0; i < possible.length && branches < samplePerBoard * active.length; i += step) {
      ...
      if (filtered.length > 1) {
        ...
        for (const fc of cands) {
          const { adj } = bucketAdj(fc, filtered);
          bestFollow = Math.min(bestFollow, Math.ceil(adj) || 1);
        }
        totalRem += bestFollow;
...
function wbGetBest(boards, guessesMade, topN = 5) {
  ...
  // Phase 2: recursive lookahead on top 30
...
// GAME SIMULATION
// ============================================================
function simulate(targets, getBestFn, maxGuesses = 9) {
  let possible = targets.map(() => [...answers]);
...
console.log('  30-GAME COMPARISON — Both with recursive lookahead');
console.log('  JS wordlebot-style  vs  Rust WASM solver');
console.log('═'.repeat(72));

let wbW = 0, wbG = 0, wbRes = [];
let ourW = 0, ourG = 0, ourRes = [];
...
for (let i = 0; i < puzzles.length; i++) {
  ...
  const wb = simulate(p, wbGetBest, 9);
  const wbMs = performance.now() - t1;

  const t2 = performance.now();
  const our = simulate(p, (boards, gm, tn) => {
    const r = solver.getQuickBestGuesses(boards, gm, tn);
  ...
  const wbS = wb.success ? `${wb.guesses}✓` : 'X';
  const ourS = our.success ? `${our.guesses}✓` : 'X';
  const w = wb.success && our.success
    ? (wb.guesses < our.guesses ? '← wb' : our.guesses < wb.guesses ? '← OURS' : 'tie')
    : wb.success ? '← wb' : our.success ? '← OURS' : 'both X';
  console.log(`wb:${wbS}(${wbMs.toFixed(0)}ms) | our:${ourS}(${ourMs.toFixed(0)}ms) ${w}`);
}

console.log('\n' + '═'.repeat(72));
console.log('  FINAL RESULTS (30 games)');
console.log('═'.repeat(72));

const wbAvg = wbW > 0 ? (wbG / wbW) : Infinity;
const ourAvg = ourW > 0 ? (ourG / ourW) : Infinity;

console.log(`\n  JS WORDBOT:    ${wbW}/30 wins | avg ${wbAvg.toFixed(2)} guesses`);
console.log(`  RUST WASM:     ${ourW}/30 wins | avg ${ourAvg.toFixed(2)} guesses`);

let better = 0, worse = 0, tie = 0;
for (let i = 0; i < 30; i++) {
  const w = wbRes[i], o = ourRes[i];
  if (w.success && o.success) {
    if (w.guesses < o.guesses) worse++;
    else if (o.guesses < w.guesses) better++;
    else tie++;
  } else if (w.success) worse++;
  else if (o.success) better++;
  else tie++;
}
console.log(`\n  H2H: wordlebot wins: ${worse} | our wins: ${better} | tied: ${tie}`);
const champ = ourAvg < wbAvg ? '🏆 OUR RUST WASM SOLVER' : wbAvg < ourAvg ? '🏆 JS WORDBOT' : '🏆 TIE';
console.log(`\n  ${champ} (${Math.abs(ourAvg - wbAvg).toFixed(2)} avg difference)`);
